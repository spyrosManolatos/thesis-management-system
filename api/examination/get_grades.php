...
<?php
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            SELECT quality_completeness, targets_fulfiled, readable_thesis, time_satisfied
            FROM marks
            ...
            WHERE teacher.username = :t_username AND thesis_assignments.thesis_assignment_id = :th_as_id
        ");
        $stmt->bindParam(":t_username", $_SESSION['username'], PDO::PARAM_STR);
        $stmt->bindParam(":th_as_id", $assignmentId, PDO::PARAM_INT);
        $stmt->execute();
        $specificmarks = $stmt->fetchAll(PDO::FETCH_ASSOC);


        

        echo json_encode($specificmarks);
